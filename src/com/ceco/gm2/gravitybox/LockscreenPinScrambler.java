...
public class LockscreenPinScrambler {
    ...
    private static final String CLASS_NUMPAD_KEY = "com.android.internal.policy.impl.keyguard.NumPadKey";
    ...
    private void changeDigit(Object key, int digit) {
        ...
        if (digit >= 0) {
            ...
            if (sKlondike != null && sKlondike.length > digit) {
                final String extra = sKlondike[digit];
                final int extraLen = extra.length();
                if (extraLen > 0) {
                    builder.append(" ");
                    builder.append(extra);
                    builder.setSpan(
                        new TextAppearanceSpan(mContainer.getContext(), 
                                getTextApearanceStyleId()),
                        builder.length()-extraLen, builder.length(), 0);
                }
            }
        }
        ((TextView)key).setText(builder);
    }

    private int getTextApearanceStyleId() {
        return mContainer.getResources()
                .getIdentifier("TextAppearance.NumPadKey.Klondike", "style",
                        "android");
    }
}
