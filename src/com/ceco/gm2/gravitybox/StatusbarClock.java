...
public class StatusbarClock implements IconManagerListener, BroadcastSubReceiver {
    ...
    public void setClock(TextView clock) {
        ...
        mPm = (PowerManager) mClock.getContext().getSystemService(Context.POWER_SERVICE);
        ...
        mClock.addOnAttachStateChangeListener(new OnAttachStateChangeListener() {
            ...
            public void onViewAttachedToWindow(View v) {
                updateSecondsHandler(true);
            }
            @Override
            public void onViewDetachedFromWindow(View v) {
                updateSecondsHandler(false);
            }
        });
    ...
    private void updateSecondsHandler(boolean attached) {
        if (mClock == null) return;

        if (mShowSeconds && attached) {
            mSecondsHandler = new Handler();
            if (mPm.isScreenOn() && !mClockHidden) {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (mShowSeconds && sbClock != null) {
                        if (mSecondsFormat == null) {
                            mSecondsFormat = new SimpleDateFormat(is24 ? "HH:mm:ss" : "hh:mm:ss",
                                    Locale.getDefault());
                        }
