...
public class ModNavigationBar {
    ...
    private static int mRecentsSingletapActionBck = 0;
    private static int mRecentsLongpressActionBck = 0;
    ...
    private static void broadcastRecentsActions(Context context, int singleTapAction, int longPressAction) {
        if (context == null) return;
        ...
        try {
            ...
            for (int i = 0; i < (12 - dummySlotCount); i++) {
                if (i < GravityBoxSettings.PREF_KEY_NAVBAR_RING_TARGET.size()) {
                    ...
                    newTargets.add(GlowPadHelper.createTargetDrawable(context, ai));
    ...
    private static void updateRingTarget(int index, String app) {
        ...
        try {
            ...
            if (targets != null && targets.size() > index) {
                targets.set(index, GlowPadHelper.createTargetDrawable(context, ai));
